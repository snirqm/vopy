...
class PublicService(PublicApiStub):
    ...
    async def get_status(self, empty: Empty) -> Status:
        return Status(open_simulations=self.get_open_simulations(),
    ...
    async def start_sim(self, session: Session) -> SimulationInfo:
        ...
        return SimulationInfo(port=port, session=session)
    ...
    def make_dir(dir_path: str) -> None:
        if not os.path.exists(dir_path):
            ...
            raise GRPCError(status=grpclib.const.Status.ALREADY_EXISTS,
    ...
    def check_session_exists(self, key: int) -> None:
        if key not in self.sessions_info_dict:
            raise GRPCError(status=grpclib.const.Status.FAILED_PRECONDITION,
